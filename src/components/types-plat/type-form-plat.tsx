...
import { createTypePlat, updateTypePlat } from "@/hooks/types_plat";
...
export default function MomentForm(
  { cat, intent }: { cat : Type_plat, intent: string }
) {
  ...
  const onSubmit: SubmitHandler<Inputs> = async ({ label }) => {
    ...
    switch (intent) {
      ...
      case (INTENT.update):
        try {
          await updateTypePlat({ id: cat.id!, label });
          router.push("/recettes/types-de-plat");
  ...
  return (
    <div className="cats__container">
      <h3 className='cats__title'>{intent === 'create' ? 'Ajouter' : 'Editer'} un type de plat</h3>
      ...
      <BackBtn url="types-de-plat" label="types de plat" />
    </div>
  )
}