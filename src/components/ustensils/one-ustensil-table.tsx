...
import { deleteUstensil } from "@/hooks/ustensils";

import './ustensil.css'

export default function OneUstensilTable({ cat }: { cat: Ustensil }) {
  ...
  return (
    <div className="cats__container">
      ...
      <div className="cats__one-cat__options">
        <Link className="cats__table__editbtn" href={`/recettes/ustensils/edit/${cat.id}`}>
          Editer
        </Link>
        <div className="cats__table__deleteBtn" onClick={handleDelete}>Supprimer</div>
        <Link className="button" href="/recettes/ustensils">Retour aux ustensils</Link>
      </div>
    </div>
  )
}