...
import { Complexite, Couleur_plat, Ingredient, IngredientWithQuantityAndUnity, PlatComplet, Regime_alimentaire, Repas, Saveur, Type_plat, Ustensil } from "@/types/recettes.types";
import { createPlat, updatePlat } from "@/hooks/plats";
...
export default function EditPlatForm({
  ...
  const [chosenRepas, setChosenRepas] = useState<Repas[]>(plat.repas ?? []);
  const [chosenCouleurs, setChosenCouleurs] = useState<Couleur_plat[]>(plat.couleurs_plat ?? []);
  const [chosenRegimes, setChosenRegimes] = useState<Regime_alimentaire[]>(plat.regimes_alimentaire ?? []);
  const [chosenSaveurs, setChosenSaveurs] = useState<Saveur[]>(plat.saveurs ?? []);
  const [chosenUstensils, setChosenUstensils] = useState<Ustensil[]>(plat.ustensils ?? []);
  const [chosenIngredientsWithDetails, setChosenIngredientsWithDetails] =
    useState<IngredientWithQuantityAndUnity[]>(plat.ingredients ?? []);
  
  const [showModalIngredient, setShowModalIngredient] = useState(false);
  const [cuissons, setCuissons] = useState<Cuisson[]>(plat.cuisson ?? [])
  const [saisons, setSaisons] = useState<Saison[]>(plat.saison ?? [])
  ...
  return (
    <div className="cats__container">
      <h3 className='cats__title'>Editer un plat</h3>
